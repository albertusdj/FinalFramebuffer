...
int findIntersect (int is_y_known, int point_idx, int known_val, int x1, int y1, int x2, int y2, polygon frame) {
...
void add_point_to_polygon (polygon *res, int x, int y, int debug) {
	if (debug) {
		printf("\nadding to polygon\n");
	}
...
void clip (polygon * res, int x1, int y1, int x2,int y2, polygon frame, int debug) {
	...
	add_point_to_polygon(res, x1, y1, debug);
	add_point_to_polygon(res, x2, y2, debug);
...
polygon dilate(polygon pol, polygon frame, int scale, int x_center, int y_center){
    ...
    for (int i = 0; i < (pol.size - 1); i++) {
        ...
        // clip(&res, x1, y1, x2, y2, frame, debug);
        
		// printf("Mapping to frame (%d - %d, %d - %d)\n",frame.arr[0][0],frame.arr[1][0],frame.arr[0][1],frame.arr[2][1]);


		// if (x1 < frame.arr[0][0]) {
		// 	// add_point_to_polygon(&res, frame.arr[0][0], getY())
		// 	add_point_to_polygon(&res, frame.arr[0][0],findIntersect(0,1,frame.arr[0][0],x1,y1,x2,y2,frame));
		// } else if (x1 > frame.arr[1][0]) {
		// 	add_point_to_polygon(&res, frame.arr[1][0],findIntersect(0,1,frame.arr[1][0],x1,y1,x2,y2,frame));
		// } else {
		// 	if (y1 < frame.arr[0][1]) {
		// 		add_point_to_polygon(&res, findIntersect(1,1,frame.arr[0][1],x1,y1,x2,y2,frame),frame.arr[0][1]);
		// 	} else if (y1 > frame.arr[2][1]) {
		// 		add_point_to_polygon(&res, findIntersect(1,1,frame.arr[2][1],x1,y1,x2,y2,frame),frame.arr[2][1]);
		// 	} else {
		...
		// if (x2 < frame.arr[0][0]) {
		// 	add_point_to_polygon(&res, frame.arr[0][0],findIntersect(0,2,frame.arr[0][0],x1,y1,x2,y2,frame));
		// } else if (x2 > frame.arr[1][0]) {
		// 	add_point_to_polygon(&res, frame.arr[1][0],findIntersect(0,2,frame.arr[1][0],x1,y1,x2,y2,frame));
		// } else {
		// 	if (y2 < frame.arr[0][1]) {
		// 		add_point_to_polygon(&res, findIntersect(1,2,frame.arr[0][1],x1,y1,x2,y2,frame),frame.arr[0][1]);
		// 	} else if (y2 > frame.arr[2][1]) {
		// 		add_point_to_polygon(&res, findIntersect(1,2,frame.arr[2][1],x1,y1,x2,y2,frame),frame.arr[2][1]);
		// 	} else {
...
polygon translate(polygon pol, polygon frame, int x_distance, int y_distance, int debug){
	polygon res;
